...
CREATE TABLE IF NOT EXISTS obligating_decisions (
    ...
    utworzono_o TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    zaktualizowano_o TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (recommendation_kod_zalecenia) REFERENCES recommendations(kod_zalecenia) ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (nazwa_podmiotu_id) REFERENCES slownik_pozycje(id),
...
CREATE_RISK_EXPOSURE_TEAMS_TABLE_SQL = """
CREATE TABLE IF NOT EXISTS risk_exposure_teams (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    risk_exposure_id INTEGER NOT NULL,
    team_id INTEGER NOT NULL,
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (risk_exposure_id) REFERENCES risk_exposure_requests(id) ON DELETE CASCADE,
    FOREIGN KEY (team_id) REFERENCES teams(id),
    UNIQUE (risk_exposure_id, team_id)
)
"""

CREATE_INDEX_RISK_EXPOSURE_TEAMS_REQ_SQL = """
CREATE INDEX IF NOT EXISTS idx_risk_exposure_teams_req
ON risk_exposure_teams(risk_exposure_id)
"""

CREATE_INDEX_RISK_EXPOSURE_TEAMS_TEAM_SQL = """
CREATE INDEX IF NOT EXISTS idx_risk_exposure_teams_team
ON risk_exposure_teams(team_id)
"""
...
CREATE_UNIQUE_INDEX_USERS_LOGIN_NOCASE_SQL = """
CREATE UNIQUE INDEX IF NOT EXISTS idx_users_login_nocase_unique
...
CREATE TABLE IF NOT EXISTS notification_schedules (
    ...
    control_type TEXT NOT NULL DEFAULT 'empty_field' CHECK (control_type IN ('empty_field', 'status_equals')),
    status_field_code TEXT,
    expected_status_code TEXT,
    expected_status_codes TEXT,
    days_comparison_mode INTEGER NOT NULL DEFAULT 0 CHECK (days_comparison_mode IN (0, 1, 2)),
...
WITH seed(kod_typu, kod_pozycji, nazwa_pozycji, skrot_pozycji, kolejnosc, aktywny) AS (
    VALUES
...
SELECT s.kod_typu, s.kod_pozycji, s.nazwa_pozycji, s.skrot_pozycji, s.kolejnosc, s.aktywny
FROM seed s
WHERE NOT EXISTS (
    SELECT 1
    FROM slownik_pozycje p
    WHERE lower(trim(p.kod_typu)) = lower(trim(s.kod_typu))
      AND upper(trim(p.kod_pozycji)) = upper(trim(s.kod_pozycji))
)
"""
...
INSERT INTO slownik_pozycje
    (kod_typu, kod_pozycji, nazwa_pozycji, skrot_pozycji, kolejnosc, aktywny)
SELECT 'department_ogolne', 'DIU', 'DIU', 'DIU', 1, 1
WHERE NOT EXISTS (
    SELECT 1
    FROM slownik_pozycje p
    WHERE lower(trim(p.kod_typu)) = 'department_ogolne'
      AND upper(trim(p.kod_pozycji)) = 'DIU'
)
"""
...
WHERE u.aktywny = 1
    ...
    AND NOT EXISTS (
        SELECT 1
        FROM slownik_pozycje p
        WHERE lower(trim(p.kod_typu)) = 'osoby'
          AND upper(trim(p.kod_pozycji)) = upper('OSOBA_' || CAST(u.id AS TEXT))
    )
"""
...
WHERE NOT EXISTS (
    SELECT 1
    FROM slownik_pozycje p
    WHERE lower(trim(p.kod_typu)) = 'rozstrzygniecie_wniosku_sankcyjnego_i'
      AND upper(trim(p.kod_pozycji)) = 'I'
)
"""
...
def _ensure_users_schema(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(CREATE_UNIQUE_INDEX_USERS_LOGIN_NOCASE_SQL)
...
def _ensure_notification_schedules_schema(conn: sqlite3.Connection) -> None:
    ...
    if "control_type" not in schedule_columns:
        conn.execute("ALTER TABLE notification_schedules ADD COLUMN control_type TEXT NOT NULL DEFAULT 'empty_field'")
    if "status_field_code" not in schedule_columns:
        conn.execute("ALTER TABLE notification_schedules ADD COLUMN status_field_code TEXT")
    if "expected_status_code" not in schedule_columns:
        conn.execute("ALTER TABLE notification_schedules ADD COLUMN expected_status_code TEXT")
    if "expected_status_codes" not in schedule_columns:
        conn.execute("ALTER TABLE notification_schedules ADD COLUMN expected_status_codes TEXT")
    if "days_comparison_mode" not in schedule_columns:
        conn.execute("ALTER TABLE notification_schedules ADD COLUMN days_comparison_mode INTEGER NOT NULL DEFAULT 0")

    conn.execute(
        """
        UPDATE notification_schedules
        SET days_comparison_mode = 0
        WHERE days_comparison_mode IS NULL
           OR days_comparison_mode NOT IN (0, 1, 2)
        """
    )
    ...
    conn.execute(
        """
        UPDATE notification_schedules
        SET expected_status_codes = upper(trim(expected_status_code))
        WHERE (expected_status_codes IS NULL OR trim(expected_status_codes) = '')
          AND expected_status_code IS NOT NULL
          AND trim(expected_status_code) <> ''
        """
    )
...
CANONICAL_INSPECTION_STATUSES: tuple[tuple[str, str, str, int], ...] = (
    ("I_SI_1", "inspekcja w przygotowaniu", "przygotowanie", 1),
    ("I_SI_2", "trwaja czynnosci inspekcyjne", "trwa", 2),
    (
        "I_SI_3",
        "czynnosci inspekcyjne zakonczone - opracowywanie protokolu/sprawozdania",
        "zakonczona - piszemy",
        3,
    ),
    ("I_SI_4", "protokol dostarczono - oczekujemy na zastrzezenia", "protokol dostarczono", 4),
    (
        "I_SI_5",
        "przekazano pismo z ustaleniami po wizycie - oczekujemy na odpowiedz",
        "przekazano pismo po wizycie",
        5,
    ),
    ("I_SI_6", "wplynely zastrzezenia - przygotowanie odpowiedzi", "wplynely zastrzezenia", 6),
    ("I_SI_7", "wplynela odpowiedz do ustalen po wizycie", "wplynela odp. po wizycie", 7),
    (
        "I_SI_8",
        "opracowywanie rekomendacji dalszych dzialan/zalecen",
        "piszemy zalecenia/odstapienie",
        8,
    ),
    ("I_SI_9", "zamkniete - wydano zalecenia", "zamkniete - wydano zalecenia", 9),
    (
        "I_SI_10",
    ...
    (
        "I_SI_11",
        "sporzadzono sprawozdanie z wizyty nadzorczej - opracowywanie pisma do podmiotu z ustaleniami",
        "sporzadzono sprawozdanie - piszemy pismo z ustaleniami",
        11,
    ),
    ("I_SI_12", "zamkniete - sporzadzono wniosek sankcyjny", "zamkniete - sporzadzono wniosek sankcyjny", 12),
    (
        "I_SI_13",
    ...
    ("I_SI_14", "protokol sporzadzono - wyslano/planowane dostarczenie", "protokol sporzadzono", 14),
)


def _ensure_canonical_inspection_statuses(conn: sqlite3.Connection) -> None:
    target_codes = {code for code, _, _, _ in CANONICAL_INSPECTION_STATUSES}

    rows = conn.execute(
        """
        SELECT id, kod_pozycji
        FROM slownik_pozycje
        WHERE lower(kod_typu) = 'statusy_inspekcji'
        """
    ).fetchall()
    by_code: dict[str, int] = {str(row["kod_pozycji"]).upper(): int(row["id"]) for row in rows}

    for code, name, short_name, order_value in CANONICAL_INSPECTION_STATUSES:
        existing_id = by_code.get(code)
        if existing_id is None:
            conn.execute(
                ...
                VALUES
                    ('statusy_inspekcji', ?, ?, ?, ?, 1)
                """,
                (code, name, short_name, int(order_value)),
            )
        else:
            conn.execute(
                """
                ...
                WHERE id = ?
                """,
                (name, short_name, int(order_value), int(existing_id)),
            )
    ...
    canonical_rows = conn.execute(
        ...
        """.format(placeholders=", ".join("?" for _ in target_codes)),
        tuple(sorted(target_codes)),
    ).fetchall()
    canonical_id_by_code: dict[str, int] = {
        str(row["kod_pozycji"]).upper(): int(row["id"]) for row in canonical_rows
    }

    # Preserve existing inspection records while retiring legacy status codes.
    legacy_to_canonical_code: dict[str, str] = {
        "I_SI_15": "I_SI_9",
        "I_SI_16": "I_SI_10",
        "I_SI_17": "I_SI_12",
        "I_SI_18": "I_SI_13",
    ...
    legacy_rows = conn.execute(
        ...
        FROM slownik_pozycje
        WHERE lower(kod_typu) = 'statusy_inspekcji'
          AND upper(kod_pozycji) NOT IN ({placeholders})
        """.format(placeholders=", ".join("?" for _ in target_codes)),
        tuple(sorted(target_codes)),
    ).fetchall()

    default_status_id = canonical_id_by_code.get("I_SI_1")
    for legacy in legacy_rows:
        legacy_id = int(legacy["id"])
        legacy_code = str(legacy["kod_pozycji"] or "").upper()
        mapped_code = legacy_to_canonical_code.get(legacy_code, "I_SI_1")
        replacement_status_id = canonical_id_by_code.get(mapped_code, default_status_id)
        if replacement_status_id is None:
            continue
        conn.execute(
            "UPDATE inspections SET status_inspekcji_id = ? WHERE status_inspekcji_id = ?",
            (int(replacement_status_id), legacy_id),
        )
...
def _ensure_slowniki_schema(conn: sqlite3.Connection, *, seed_slownik_pozycje: bool) -> None:
    ...
    _ensure_canonical_inspection_statuses(conn)
        ...
        if required.issubset(columns):
            ...
            for fk_row in fk_rows:
                if (
                    ...
                    and (
                        str(fk_row["on_delete"] or "").upper() != "CASCADE"
                        or str(fk_row["on_update"] or "").upper() != "CASCADE"
                    )
                ):
                    rebuild_for_fk = True
                    break
...
def _ensure_risk_exposure_schema(conn: sqlite3.Connection) -> None:
    ...
    conn.execute(CREATE_RISK_EXPOSURE_TEAMS_TABLE_SQL)
    conn.execute(CREATE_INDEX_RISK_EXPOSURE_TEAMS_REQ_SQL)
    conn.execute(CREATE_INDEX_RISK_EXPOSURE_TEAMS_TEAM_SQL)
