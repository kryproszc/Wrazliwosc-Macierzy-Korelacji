...
type ReportExportFormat = "html" | "pdf";
type ReportExportDestination = "disk" | "email";

function buildSuggestedExportFileBaseName() {
	const now = new Date();
	...
	return `dashboard_${day}-${month}-${year}_${hours}-${minutes}`;
}

function normalizeExportFileBaseName(value: string) {
	const trimmed = value.trim();
	const withoutExtension = trimmed.replace(/\.(html|pdf)$/i, "");
	...
	if (!normalized) {
		return buildSuggestedExportFileBaseName();
	}
...
function inlineComputedStylesRecursively(source: Element, target: Element) {
	const sourceStyle = window.getComputedStyle(source);
	const styleText = Array.from(sourceStyle)
		.map((propertyName) => `${propertyName}:${sourceStyle.getPropertyValue(propertyName)};`)
		.join("");
	...
	for (let index = 0; index < childCount; index += 1) {
		...
		inlineComputedStylesRecursively(sourceChild, targetChild);
	}
}

function minifyExportHtmlDocument(html: string) {
	return html
		.replace(/>\s+</g, "><")
		.replace(/\n+/g, "")
		.trim();
...
function buildExportClone(sourceElement: HTMLElement) {
	const clonedElement = sourceElement.cloneNode(true) as HTMLElement;
	inlineComputedStylesRecursively(sourceElement, clonedElement);
	...
	clonedElement.querySelectorAll("*").forEach((element) => {
		...
		const isMainTableScrollArea = className.includes("table-scroll-gutter-right");
		const hasTableDescendant = element.querySelector("table") !== null;

		if (hasTableDescendant && className.includes("overflow-hidden")) {
		...
		if (className.includes("welcome-scroll-subtle")) {
			const htmlElement = element as HTMLElement;
		...
		if (className.includes("subtle-vertical-scroll") && !isMainTableScrollArea) {
			const htmlElement = element as HTMLElement;
			htmlElement.style.overflow = "visible";
			htmlElement.style.maxHeight = "none";
			htmlElement.style.height = "auto";
...
function buildReportHtmlDocument(sectionHtmlBlocks: string[]) {
	const generatedAt = new Date().toISOString();
	const title = "Raport inspekcje i zalecenia";

	const combinedSections = sectionHtmlBlocks
		.map(
			(sectionHtml, index) =>
				`<section style=\"margin:${index === 0 ? "0" : "20px 0 0"};\">${sectionHtml}</section>`,
		)
		.join("\n");
	...
	return [
		...
		"<style>",
		"html, body { margin: 0; padding: 0; background: #0f1f39; }",
		"* { box-sizing: border-box; }",
		"</style>",
		"</head>",
		"<body>",
		`<main data-generated-at=\"${escapeHtml(generatedAt)}\">`,
		combinedSections,
		"</main>",
		"</body>",
		"</html>",
...
async function openPrintDialogForPdf(html: string) {
	const frame = document.createElement("iframe");
	frame.setAttribute("data-export-ignore", "true");
	...
	await new Promise<void>((resolve, reject) => {
		...
		frame.onerror = () => reject(new Error("Nie udało się przygotować podglądu PDF."));
		frame.srcdoc = html;
	});
	...
	if (!frameWindow) {
		frame.remove();
		throw new Error("Nie udało się otworzyć podglądu wydruku PDF.");
	}
...
async function loadImageFromDataUrl(dataUrl: string) {
	return new Promise<HTMLImageElement>((resolve, reject) => {
		...
		image.onerror = () => reject(new Error("Nie udało się wczytać obrazu raportu do PDF."));
		image.src = dataUrl;
	});
}
...
async function buildPdfBlobFromHtmlSnapshot(html: string) {
	const [{ default: domToImage }, jsPdfModule] = await Promise.all([
	...
	const parsedMain = parsedDocument.querySelector("main");
	if (!parsedMain) {
		throw new Error("Brak sekcji raportu do eksportu PDF.");
	}

	const tempMount = document.createElement("div");
	tempMount.setAttribute("data-export-ignore", "true");
	tempMount.style.position = "fixed";
	tempMount.style.left = "-100000px";
	tempMount.style.top = "0";
	tempMount.style.width = "1900px";
	tempMount.style.background = "#0f1f39";
	tempMount.style.zIndex = "-1";

	const pdfRoot = document.createElement("div");
	pdfRoot.style.background = "#0f1f39";
	pdfRoot.style.padding = "8px";
	pdfRoot.appendChild(document.importNode(parsedMain, true));
	tempMount.appendChild(pdfRoot);
	document.body.appendChild(tempMount);

	try {
		await waitForNextPaint();

		const pdfRootRect = pdfRoot.getBoundingClientRect();
		const rowBoundaryOffsetsPx = Array.from(
			pdfRoot.querySelectorAll<HTMLTableRowElement>("tbody tr"),
		)
			.map((rowElement) => {
				const rowRect = rowElement.getBoundingClientRect();
				return Math.round(rowRect.bottom - pdfRootRect.top);
			})
			.filter((offset) => Number.isFinite(offset) && offset > 0)
			.sort((left, right) => left - right);

		const widthPx = Math.max(pdfRoot.scrollWidth, 1200);
		const heightPx = Math.max(pdfRoot.scrollHeight, 800);
		const imageDataUrl = await domToImage.toPng(pdfRoot, {
			bgcolor: "#0f1f39",
			quality: 1,
			width: widthPx,
			height: heightPx,
		...
		const scalePtPerPx = renderedWidth / image.width;
		const pageHeightPx = Math.max(1, Math.floor(pageHeight / scalePtPerPx));
		...
		if (!rasterContext) {
			throw new Error("Nie udało się przygotować podziału stron PDF.");
		}
		rasterContext.drawImage(image, 0, 0);
		const rasterData = rasterContext.getImageData(0, 0, image.width, image.height).data;

		const rowDarkPixelCount = (rowIndex: number) => {
			const clampedRow = Math.max(0, Math.min(image.height - 1, rowIndex));
			const rowStart = clampedRow * image.width * 4;
			let darkPixels = 0;

			for (let x = 0; x < image.width; x += 3) {
				const pixelOffset = rowStart + x * 4;
				const alpha = rasterData[pixelOffset + 3] ?? 0;
				if (alpha < 24) {
					continue;
				}

				const red = rasterData[pixelOffset] ?? 255;
				const green = rasterData[pixelOffset + 1] ?? 255;
				const blue = rasterData[pixelOffset + 2] ?? 255;
		...
		const findPreferredRowBoundary = (
			searchStartPx: number,
			searchEndPx: number,
			targetBreakPx: number,
		) => {
			let bestCandidate: number | null = null;
			for (const boundary of rowBoundaryOffsetsPx) {
				if (boundary < searchStartPx) {
					continue;
				}

				if (boundary > searchEndPx) {
					break;
				}

				if (
					bestCandidate === null ||
					Math.abs(boundary - targetBreakPx) < Math.abs(bestCandidate - targetBreakPx)
				) {
		...
		const pageBreakSearchRadiusPx = 90;
		const minSliceHeightPx = Math.max(120, Math.floor(pageHeightPx * 0.7));
		const boundaries: number[] = [0];
		let currentTopPx = 0;

		while (currentTopPx + pageHeightPx < image.height) {
			const targetBreakPx = currentTopPx + pageHeightPx;
			const searchStartPx = Math.max(
				currentTopPx + minSliceHeightPx,
				targetBreakPx - pageBreakSearchRadiusPx,
			);
			...
			const rowBoundaryBreakPx = findPreferredRowBoundary(
				searchStartPx,
				searchEndPx,
				targetBreakPx,
			);
			...
			if (rowBoundaryBreakPx === null) {
				let bestScore = Number.POSITIVE_INFINITY;

				for (let rowIndex = searchStartPx; rowIndex <= searchEndPx; rowIndex += 1) {
					const score = rowDarkPixelCount(rowIndex);
					if (
						score < bestScore ||
						(score === bestScore &&
							Math.abs(rowIndex - targetBreakPx) < Math.abs(bestBreakPx - targetBreakPx))
					) {
						bestScore = score;
						bestBreakPx = rowIndex;
					}
				}
			} else {
				bestBreakPx = rowBoundaryBreakPx;
			}

			const nextBreakPx = Math.max(
				currentTopPx + minSliceHeightPx,
				Math.min(bestBreakPx, image.height - 1),
			);
			boundaries.push(nextBreakPx);
			currentTopPx = nextBreakPx;
		}

		if (boundaries[boundaries.length - 1] !== image.height) {
			boundaries.push(image.height);
		}

		const totalPages = boundaries.length - 1;
		for (let pageIndex = 0; pageIndex < totalPages; pageIndex += 1) {
			const sourceTopPx = boundaries[pageIndex] ?? 0;
			const sourceBottomPx = boundaries[pageIndex + 1] ?? image.height;
			const sliceHeightPx = Math.max(1, sourceBottomPx - sourceTopPx);
			...
			if (!sliceContext) {
				throw new Error("Nie udało się przygotować strony PDF.");
			}
			...
			const renderHeightPt = isLastPage ? sliceHeightPx * scalePtPerPx : pageHeight;
...
async function savePdfFromHtmlSnapshot(html: string, fileName: string) {
	const pdfBlob = await buildPdfBlobFromHtmlSnapshot(html);
...
export function WelcomeStartPanel({ operatorLogin }: WelcomeStartPanelProps) {
	const dashboardSnapshotRef = useRef<HTMLElement | null>(null);
	...
	const [isExportFormatPickerOpen, setIsExportFormatPickerOpen] = useState(false);
	const [selectedExportFormat, setSelectedExportFormat] = useState<ReportExportFormat>("html");
	const [selectedExportDestination, setSelectedExportDestination] =
		useState<ReportExportDestination>("disk");
	const [exportFileBaseName, setExportFileBaseName] = useState(() =>
		buildSuggestedExportFileBaseName(),
	);
	const [exportActionError, setExportActionError] = useState<string | null>(null);
	const [exportActionSuccess, setExportActionSuccess] = useState<string | null>(null);
	...
	const handleOpenExportOptions = useCallback(() => {
		...
		setExportActionSuccess(null);
		setExportFileBaseName(buildSuggestedExportFileBaseName());
		setIsExportFormatPickerOpen(true);
	}, []);
	...
	const handleConfirmExport = useCallback(async () => {
		...
		const sourceElement = dashboardSnapshotRef.current;
		if (!sourceElement) {
			return;
		}
		...
		const previouslyActiveSection = activeTopSection;

		const captureSectionHtml = async (
			section: "inspections" | "recommendations",
			label: string,
		) => {
			...
			const clonedElement = buildExportClone(sourceElement);
			return [
				`<h2 style=\"margin:0 0 10px;font-family:Segoe UI,Tahoma,sans-serif;font-size:18px;font-weight:700;color:#e2e8f0;\">${escapeHtml(label)}</h2>`,
				clonedElement.outerHTML,
			].join("\n");
		};

		try {
			const inspectionsHtml = await captureSectionHtml("inspections", "Inspekcje");
			const recommendationsHtml = await captureSectionHtml("recommendations", "Zalecenia");

			const html = buildReportHtmlDocument([inspectionsHtml, recommendationsHtml]);
			const minifiedHtml = minifyExportHtmlDocument(html);
			const normalizedFileBaseName = normalizeExportFileBaseName(exportFileBaseName);
			const htmlFileName = `${normalizedFileBaseName}.html`;
			const pdfFileName = `${normalizedFileBaseName}.pdf`;

			if (selectedExportDestination === "email") {
				if (selectedExportFormat !== "pdf") {
					setExportActionError(
						"Dla wysyłki e-mail dostępny jest tylko format PDF.",
					);
					return;
				}
				...
				const generatedAt = new Date().toISOString();
				const requestedFileName = pdfFileName;
				const pdfBlob = await buildPdfBlobFromHtmlSnapshot(minifiedHtml);
				const pdfArrayBuffer = await pdfBlob.arrayBuffer();
				const pdfBase64 = arrayBufferToBase64(pdfArrayBuffer);

				const result = await sendHtmlReportEmail(operatorLogin, {
					toEmails: [],
					subject: "",
					bodyText: "",
					fileName: requestedFileName,
					pdfBase64,
					reportType: activeTopSection,
					meta: {
						generatedAt,
						requestedFileFormat: "pdf",
						activeTopSection,
						includedSections: ["inspections", "recommendations"],
						filters: {
							selectedInspectionStageFilters: selectedStageFilters,
							selectedRecommendationStatuses,
						},
						sender: {
							login: session?.user?.login ?? operatorLogin,
							imie: session?.user?.imie ?? null,
							nazwisko: session?.user?.nazwisko ?? null,
							rola: session?.user?.rola ?? null,
						},
					},
				});
				...
				if (!result.ok) {
					setExportActionError(
						result.status === 404
							? `${result.error} Brakuje endpointu POST /api/reports/send-html po stronie backendu.`
							: result.error,
					);
					return;
				}

				setIsExportFormatPickerOpen(false);
				setExportActionSuccess("Plik PDF został wysłany na Twój adres e-mail.");
				return;
			}
			...
			if (selectedExportFormat === "html") {
				...
				try {
					await savePdfFromHtmlSnapshot(
						minifiedHtml,
						pdfFileName,
					);
				} catch {
					await openPrintDialogForPdf(minifiedHtml);
				}
			}
		} finally {
			setActiveTopSection(previouslyActiveSection);
			await waitForNextPaint();
	...
	return (
		<section
			...
			className="relative flex h-full min-h-0 w-full flex-col py-2"
		>
			{isHtmlExporting ? (
				<div
					data-export-ignore="true"
					className="absolute inset-0 z-40 flex items-center justify-center bg-white/72 backdrop-blur-[1px]"
				>
					<div className="inline-flex items-center gap-3 rounded-lg border border-slate-300 bg-white px-4 py-3 text-slate-700 shadow-lg">
						<span
							className="h-5 w-5 animate-spin rounded-full border-2 border-slate-300 border-t-[#2b5d9a]"
							aria-hidden="true"
						/>
						<span className="font-medium text-sm">Przygotowywanie eksportu...</span>
					</div>
				</div>
			) : null}

			{isExportFormatPickerOpen && !isHtmlExporting ? (
				<div
					data-export-ignore="true"
					className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/45"
					onClick={() => setIsExportFormatPickerOpen(false)}
				>
					<div
						className="w-full max-w-2xl rounded-xl border border-slate-300 bg-white p-4 text-slate-900 shadow-[0_20px_45px_rgba(2,8,23,0.35)]"
						onClick={(event) => event.stopPropagation()}
					>
						<h3 className="mb-3 font-semibold text-base">Opcje eksportu</h3>

						<div className="mb-4">
							<p className="mb-2 font-medium text-slate-800 text-sm">Miejsce zapisu</p>
							<div className="flex flex-wrap gap-2">
								<button
									type="button"
									onClick={() => setSelectedExportDestination("disk")}
									className={`rounded-lg border px-3 py-2 font-medium text-sm transition-colors ${
										selectedExportDestination === "disk"
											? "border-[#6ea3f0] bg-[#eaf2ff] text-[#1f4f8f]"
								...
								<button
									type="button"
									onClick={() => {
										setSelectedExportDestination("email");
										setSelectedExportFormat("pdf");
									}}
									className={`rounded-lg border px-3 py-2 font-medium text-sm transition-colors ${
										selectedExportDestination === "email"
											? "border-[#6ea3f0] bg-[#eaf2ff] text-[#1f4f8f]"
											: "border-slate-300 bg-white text-slate-700 hover:bg-slate-100"
									}`}
								>
						...
						<div className="mb-4">
							<p className="mb-2 font-medium text-slate-800 text-sm">Wybór pliku</p>
							<div className="flex flex-wrap gap-2">
								<button
									type="button"
									onClick={() => setSelectedExportFormat("html")}
									disabled={selectedExportDestination === "email"}
									className={`rounded-lg border px-3 py-2 font-medium text-sm transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${
						...
						<div className="flex items-center justify-end gap-2 border-slate-200 border-t pt-3">
							<button
								type="button"
								onClick={() => setIsExportFormatPickerOpen(false)}
								className="inline-flex h-9 items-center rounded-lg border border-slate-300 bg-white px-3 font-medium text-slate-700 text-sm transition-colors hover:bg-slate-100"
							>
								Anuluj
							</button>
							<button
								...
								className="inline-flex h-9 items-center rounded-lg border border-[#6ea3f0] bg-[#285186] px-3 font-semibold text-slate-100 text-sm transition-colors hover:bg-[#3563a1]"
							>
								Akceptuj
							</button>
						</div>
			...
			{exportActionError ? (
				<p className="mb-3 rounded-lg border border-rose-300 bg-rose-50 px-3 py-2 text-rose-800 text-sm">
					{exportActionError}
				</p>
			) : null}

			{exportActionSuccess ? (
				<p className="mb-3 rounded-lg border border-emerald-300 bg-emerald-50 px-3 py-2 text-emerald-800 text-sm">
					{exportActionSuccess}
				</p>
			) : null}

			<div
				className="mb-3 flex flex-wrap items-end gap-2 border-[#2a4772] border-b"
				data-export-ignore="true"
			>
				<button
					type="button"
				...
				<div className="ml-auto pb-1">
					<button
						...
						data-export-ignore="true"
						className="inline-flex h-8 items-center rounded-lg border border-[#6ea3f0] bg-[#285186] px-3 font-semibold text-slate-100 text-sm transition-colors hover:bg-[#3563a1] disabled:cursor-not-allowed disabled:opacity-70"
					>
						{isHtmlExporting ? "Eksportowanie..." : "Eksportuj"}
					</button>
				</div>
			</div>
