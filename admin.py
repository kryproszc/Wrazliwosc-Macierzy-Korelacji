...
ADMIN_USER_LOGIN_CONFLICT_CODE = "ADMIN_USER_LOGIN_CONFLICT"
ADMIN_USER_EDIT_FORBIDDEN_NOT_OWNER_CODE = "ADMIN_USER_EDIT_FORBIDDEN_NOT_OWNER"
...
class UserRead(BaseModel):
    ...
    editableByCurrentOperator: bool | None = None
...
def _raise_admin_login_conflict() -> None:
    raise HTTPException(
        status_code=409,
        detail=_admin_error_payload(
            code=ADMIN_USER_LOGIN_CONFLICT_CODE,
...
def _raise_admin_edit_forbidden_not_owner() -> None:
    raise HTTPException(
        status_code=403,
        detail=_admin_error_payload(
            code=ADMIN_USER_EDIT_FORBIDDEN_NOT_OWNER_CODE,
            detail="Kierownik moze edytowac tylko konta utworzone przez siebie.",
        ),
    )


def _is_login_unique_violation(exc: sqlite3.IntegrityError) -> bool:
    message = str(exc).lower()
    if "unique constraint failed" not in message:
        return False
    return "users.login" in message or "idx_users_login_nocase_unique" in message or "lower(login)" in message
...
def _map_user_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "editableByCurrentOperator": row.get("editable_by_current_operator"),
...
def _prepare_login_for_persistence(
    ...
    *,
    ...
    account_type: str,
    aktywny: int,
    technical_login_candidate: str | None = None,
) -> str:
    ...
    if allow_blank_login:
        ...
        if account_type == ACCOUNT_TYPE_TECHNICAL:
            candidate = str(technical_login_candidate or "").strip().lower()
            if candidate:
                if user_id is None:
                ...
                if conflict is not None:
                    raise HTTPException(
                        status_code=409,
                        detail="Uzytkownik techniczny o takim imieniu i nazwisku juz istnieje",
                    )
                return candidate
    ...
    if conflict is not None:
        _raise_admin_login_conflict()

    return normalized


def _normalize_login_part(raw: str) -> str:
    normalized = unicodedata.normalize("NFKD", str(raw or ""))
    ascii_only = normalized.encode("ascii", "ignore").decode("ascii")
    token = re.sub(r"[^a-zA-Z0-9]+", "_", ascii_only.strip().lower())
    token = re.sub(r"_+", "_", token).strip("_")
    return token or "x"


def _build_technical_login(imie: str, nazwisko: str) -> str:
    return f"techniczny_{_normalize_login_part(imie)}_{_normalize_login_part(nazwisko)}"
...
def list_users(
    ...
    for row in rows:
        ...
        row_dict["editable_by_current_operator"] = (
            True
            if operator_role_id == 3
            else bool(creator_id is not None and int(creator_id) == int(operator["id"]))
        )
...
@router.post(
    ...
    responses={
        409: {
            ...
            "description": "Konflikt danych, np. login juz istnieje",
        }
    },
)
def create_user(
    ...
    with get_connection() as conn:
        ...
        login = _prepare_login_for_persistence(
            ...
            account_type=next_account_type,
            aktywny=1 if payload.aktywny else 0,
            technical_login_candidate=(
                _build_technical_login(imie, nazwisko)
                if str(next_account_type).strip().lower() == ACCOUNT_TYPE_TECHNICAL
                else None
            ),
        )
...
def update_user(
    ...
    with get_connection() as conn:
        ...
        if operator_role_id == 2:
            ...
            if created_by_user_id is None or int(created_by_user_id) != int(operator["id"]):
                _raise_admin_edit_forbidden_not_owner()
...
def delete_user(
    ...
    with get_connection() as conn:
        ...
        if int(user_id) == int(operator["id"]):
            raise HTTPException(status_code=400, detail="Nie mozna usunac aktualnie zalogowanego dyrektora")
        ...
        try:
            ...
            raise HTTPException(
                status_code=409,
                detail="Nie mozna usunac uzytkownika, bo jest uzywany w innych rekordach",
            )
