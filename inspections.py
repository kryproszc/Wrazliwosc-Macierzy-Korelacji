...
_ALLOWED_FILTER_GROUPS = {"DYREKCJA", "DIU", "DNU"}
_DEPARTMENT_DIU_CODES = {"DIU", "O_D_1"}
_DEPARTMENT_DNU_CODES = {"DNU", "O_D_2"}
...
def _parse_team_filter_group_map_env(name: str) -> dict[str, str]:
    ...
    for token in re.split(r"[,;\n]", raw):
        ...
        if ":" not in item:
            continue
        team_code_raw, group_raw = item.split(":", 1)
        team_code = _normalize_ascii_upper(team_code_raw)
        group = str(group_raw or "").strip().upper()
        if not team_code or group not in _ALLOWED_FILTER_GROUPS:
            continue
        mapping[team_code] = group
    return mapping


_INSPECTIONS_TEAM_FILTER_GROUP_MAP = _parse_team_filter_group_map_env("INSPECTIONS_TEAM_FILTER_GROUP_MAP")
_INSPECTIONS_TEAM_FILTER_GROUP_DEFAULT = str(
    os.getenv("INSPECTIONS_TEAM_FILTER_GROUP_DEFAULT") or "DIU"
).strip().upper()
if _INSPECTIONS_TEAM_FILTER_GROUP_DEFAULT not in _ALLOWED_FILTER_GROUPS:
    _INSPECTIONS_TEAM_FILTER_GROUP_DEFAULT = "DIU"


def _classify_team_filter_group(team_code: str | None, team_name: str | None) -> str | None:
    normalized_code = _normalize_ascii_upper(team_code)
    if normalized_code in _INSPECTIONS_TEAM_FILTER_GROUP_MAP:
        return _INSPECTIONS_TEAM_FILTER_GROUP_MAP[normalized_code]

    if normalized_code in {"A", "B", "C", "D"}:
        return "DIU"

    normalized_name = _normalize_ascii_upper(team_name)
    combined = f"{normalized_code} {normalized_name}".strip()
    if "DYREKCJ" in combined or combined.startswith("DYR"):
        return "DYREKCJA"
    if "DNU" in combined:
        return "DNU"
    if "DIU" in combined:
        return "DIU"
    return _INSPECTIONS_TEAM_FILTER_GROUP_DEFAULT


def _load_team_filter_groups(conn: Any) -> tuple[list[dict[str, Any]], dict[int, str]]:
    rows = conn.execute(
        """
        SELECT
            t.id,
            t.kod,
            t.nazwa,
            COALESCE(sp.kod_pozycji, '') AS slownik_kod
        FROM teams t
        LEFT JOIN slownik_pozycje sp
            ON sp.id = t.slownik_pozycja_id
           AND lower(sp.kod_typu) = 'zespoly'
        ORDER BY t.id ASC
        """
    ).fetchall()
    ...
    for row in rows:
        team_id = int(row["id"])
        team_code = str(row["kod"] or "").strip()
        team_name = str(row["nazwa"] or "").strip()
        dict_code = str(row["slownik_kod"] or "").strip()
        filter_group = _classify_team_filter_group(dict_code or team_code, team_name)
        by_id[team_id] = filter_group
        items.append(
            {
                "id": team_id,
                "code": team_code,
...
def _resolve_user_filter_group(*, role_id: int, account_type: str, team_filter_group: str | None) -> str:
    if team_filter_group is not None:
        return team_filter_group
    if int(role_id) == 3:
        return "DYREKCJA"
    if int(role_id) == 1:
        return "DNU"
    if str(account_type or "").strip().lower() == "observer":
        return "DNU"
    return "DIU"
...
def _resolve_filtr_sklad_zespolu_id(*, department_code: Any, team_id: Any) -> int:
    ...
    if normalized_department in _DEPARTMENT_DIU_CODES:
        if normalized_team_id is None:
            return 0
        if normalized_team_id == 1:
            return 1
        if normalized_team_id == 2:
            return 2
        if normalized_team_id == 3:
            return 3
        if normalized_team_id == 4:
            return 4

    if normalized_department in _DEPARTMENT_DNU_CODES:
        return 5
    if normalized_department == "O_D_3":
        return 6
    if normalized_department == "O_D_4":
        return 7
    return 8
...
class InspectionPeopleOption(BaseModel):
    ...
    filtr_sklad_zespolu: str | None = None
    filtr_sklad_zespolu_id: int
    accountType: str | None = None
    filterGroup: str
    includeInFilters: bool = True
...
def _inspection_code_bucket(conn: Any, typ_inspekcji_id: int | None, poczatek_inspekcji: str) -> tuple[str, str]:
    try:
        year = str(date.fromisoformat(poczatek_inspekcji).year)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail="Niepoprawny format daty inspekcji") from exc
    ...
    return _inspection_prefix_from_typ(typ_nazwa), year
...
def list_inspection_people_options(
    ...
    with get_connection() as conn:
        ...
        _, team_group_by_id = _load_team_filter_groups(conn)
        ...
        for row in rows:
            row_dict = dict(row)
            login = str(row_dict.get("login") or "").strip()
            if login.casefold() == "admin":
                continue
            ...
            team_id = row_dict.get("zespol_id")
            team_filter_group = team_group_by_id.get(int(team_id)) if team_id is not None else None
            filter_group = _resolve_user_filter_group(
                role_id=int(row_dict.get("rola_id") or 0),
                account_type=account_type,
                team_filter_group=team_filter_group,
            )
            department_code = _norm(row_dict.get("department_code"))
            filtr_sklad_zespolu = filter_group
            filtr_sklad_zespolu_id = _resolve_filtr_sklad_zespolu_id(
                department_code=department_code,
                team_id=team_id,
            )
            if team_id is not None and team_filter_group is None:
                raise HTTPException(
                    status_code=500,
                    detail=f"Brak klasyfikacji filterGroup dla teamId={int(team_id)} przypisanego do userId={int(row_dict['id'])}",
                )
            items.append(
                {
                    "id": int(row_dict["id"]),
                    "login": login,
                    "displayName": _build_display_name(row_dict.get("imie"), row_dict.get("nazwisko"), login),
                    "active": active,
                    ...
                    "filtr_sklad_zespolu": filtr_sklad_zespolu,
                    "filtr_sklad_zespolu_id": filtr_sklad_zespolu_id,
...
def list_inspection_team_options(
    ...
    with get_connection() as conn:
        ...
        items, _ = _load_team_filter_groups(conn)
...
def update_structure_inspection(
    ...
    with get_connection() as conn:
        ...
        current_typ_inspekcji_id = int(current["typ_inspekcji_id"]) if current.get("typ_inspekcji_id") is not None else None
        next_typ_inspekcji_id = current_typ_inspekcji_id
        ...
        if typ_field_present:
            ...
            next_typ_inspekcji_id = int(typ_inspekcji_id)
        scope_field_present = (
        ...
        if "poczatekInspekcji" in fields or typ_field_present:
            current_bucket = _inspection_code_bucket(
                conn,
                current_typ_inspekcji_id,
                str(current.get("poczatek_inspekcji") or ""),
            )
            next_bucket = _inspection_code_bucket(
                conn,
                next_typ_inspekcji_id,
                str(next_start_date or ""),
            )
            if current_bucket != next_bucket:
                set_parts.append("kod_inspekcji = ?")
                values.append(
                    _next_inspection_code(
                        conn,
                        next_typ_inspekcji_id,
                        str(next_start_date or ""),
                    )
        ...
        _add_change("Kod inspekcji", current.get("kod_inspekcji"), updated.get("kod_inspekcji"))
