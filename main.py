...
def _reports_send_html_max_body_bytes() -> int:
    fallback_mb = 80
    raw_value = (
        os.getenv("REPORTS_API_MAX_BODY_MB")
        or os.getenv("REPORTS_SEND_HTML_MAX_MB")
        or str(fallback_mb)
    ).strip()
    try:
        parsed_mb = int(raw_value)
    except ValueError:
        parsed_mb = fallback_mb
    if parsed_mb < 30:
        parsed_mb = 30
...
async def lifespan(_: FastAPI):
    init_db()
    reports.run_reports_startup_checks()
...
@app.middleware("http")
async def enforce_send_html_body_limit(request: Request, call_next):
    if request.method.upper() == "POST" and request.url.path == "/api/reports/send-html":
        content_length_raw = (request.headers.get("content-length") or "").strip()
        if content_length_raw:
            try:
                content_length = int(content_length_raw)
            except ValueError:
                return JSONResponse(status_code=400, content={"detail": "Niepoprawny naglowek Content-Length"})

            max_bytes = _reports_send_html_max_body_bytes()
            if content_length > max_bytes:
                return JSONResponse(
                    status_code=413,
                    content={
                        "code": "REPORTS_SEND_HTML_PAYLOAD_TOO_LARGE",
                        "message": "Payload jest zbyt duzy dla /api/reports/send-html",
                        "limitMb": round(max_bytes / (1024 * 1024), 2),
                        "receivedMb": round(content_length / (1024 * 1024), 2),
                    },
                )
