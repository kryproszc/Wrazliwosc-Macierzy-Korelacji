...
# Local, module-only switch for temporary historical migration mode.
# Set to False to restore normal restrictions without touching other modules.
HISTORICAL_OPEN_MODE_ENABLED = True
...
def _is_historical_open_mode_enabled() -> bool:
    return HISTORICAL_OPEN_MODE_ENABLED
...
def _recommendation_status_is_blocked(
    ...
    if _is_historical_open_mode_enabled():
        return False, status_code, status_label
...
def _base_select_sql() -> str:
    return """
        ...
        LEFT JOIN users cbu ON cbu.id = d.created_by_user_id
...
def create_obligating_decision(
    ...
    with get_connection() as conn:
        ...
        if osoby_ii_ids:
            can_assign_ii_on_create = _is_historical_open_mode_enabled()
