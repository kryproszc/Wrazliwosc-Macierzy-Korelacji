...
def _year_from_iso_date(value: str | None) -> str | None:
    normalized = _norm(value)
    if normalized is None:
        return None
    try:
        return str(date.fromisoformat(normalized).year)
    except ValueError:
        return None


def _recommendation_code_year_from_code(code_value: str | None) -> str | None:
    normalized = _norm(code_value)
    if normalized is None:
        return None
...
def _base_select_sql() -> str:
    return """
        ...
        LEFT JOIN users cbu ON cbu.id = r.created_by_user_id
...
def _row_to_payload(row: dict[str, Any], lp: int, can_edit: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "createdByUserId": int(row["created_by_user_id"]) if row.get("created_by_user_id") is not None else None,
        "createdByLogin": creator_login,
        "createdByDisplayName": creator_display or creator_login,
        "canEdit": can_edit,
...
def update_recommendation(
    ...
    with get_connection() as conn:
        ...
        if single_date_payload_present:
            ...
            # Business rule: recommendation code is recalculated only when
            # dataZalecen is explicitly updated and moves to a different year.
            next_code_year = _year_from_iso_date(resolved_data_zalecen)
            current_code_year = _recommendation_code_year_from_code(current_dict.get("kod_zalecenia"))
            if next_code_year is not None and next_code_year != current_code_year:
                set_parts.append("kod_zalecenia = ?")
                values.append(_next_recommendation_code(conn, next_code_year))
